...
from _scripts import misc, ontology as og, fptree as fp
...
if symbol:
    ...
    target_genes = misc.get_values_from_keys(keys=target_genes, dictio=gene_symbol_to_id_dict)
...
min_item_weight = fp.get_numeric_input("weight", 0, 1)
min_item_freq = fp.get_numeric_input("frequency", 0, 1)
...
scatter_rules = fp.get_scatterplot(
    confidences, lifts, axes=['conf', 'lift'], title="Metrics of the association rules")
scatter_rules.show()

# set the threshold values and filter
min_rule_conf = fp.get_numeric_input("confidence", 0, 1)
min_rule_lift = fp.get_numeric_input("lift", 0, inf)
